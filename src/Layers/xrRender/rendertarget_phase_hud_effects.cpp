...
void CRenderTarget::phase_hud_blood()
{
...
#else
	u_setrt(RCache, dest_rt, ref_rt(), ref_rt(), get_base_zb());
#endif
	...
	RCache.set_c("hud_blood_params", ActorMaxHealth-ActorHealth, 0.0f, 0.0f, 0.0f);
...
void CRenderTarget::phase_hud_power()
{
	...
	RCache.set_c("hud_power_params", ActorMaxPower - ActorCurrentPower, 0.0f, 0.0f, 0.0f);
	RCache.set_Geometry(g_aa_AA);
	RCache.Render(D3DPT_TRIANGLELIST, Offset, 0, 4, 0, 2);
...
void CRenderTarget::phase_hud_bleeding()
{
	...
	RCache.set_c("hud_bleeding_params", ActorCurrentBleeding, 0.0f, 0.0f, 0.0f);
	RCache.set_Geometry(g_aa_AA);
	RCache.Render(D3DPT_TRIANGLELIST, Offset, 0, 4, 0, 2);
